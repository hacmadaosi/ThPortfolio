...
window.fillForm = (e) => {
  ...
  document.getElementById("name").value = e.TenDangNhap || "";
  document.getElementById("role").value = e.QuanLy ? "admin" : "customer";
};
...
function ManagerAction() {
  ...
  if (btnSave) {
    btnSave.addEventListener("click", async () => {
      if (!idChange) return alert("Bạn chưa chọn người dùng để sửa!");
      ...
      if (result.state) {
        ShowNotification(result.state, result.result);
        CallApiGetAllUser();
        btnCancel.click();
      }
    });
...
const UserProfileAction = () => {
  ...
  if (txtUserName) {
    txtUserName.textContent = SubText(user.Hoten, 20);
  }
