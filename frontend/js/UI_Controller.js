...
window.fillForm = (listusers) => {
  document.getElementById("email").value = listusers.Email || "";
  document.getElementById("name").value = listusers.TenDangNhap || "";
  document.getElementById("role").value = listusers.QuanLy ? "Quản lý" : "Khách hàng";

  window.currentUserId = listusers.id;
};
  ...
  if (tabTk){
    ...
    users.forEach((e, index) => {
        ...
        row.innerHTML = `
            ...
            <td class="py-2 px-4 border-b">${e.TenDangNhap || ""}</td>
            <td class="py-2 px-4 border-b">${e.QuanLy ? "Quản lý" : "Khách hàng"}</td>
            <td class="py-2 px-4 border-b">
              <button class="btnEdit text-blue-500 underline mr-2" data-id="${e.id}">
                Sửa
              </button>

              <button class="btnDelete text-red-600 underline" data-id="${e.id}">
                Xóa
              </button>
            </td>`;

        row.querySelector(".btnEdit").addEventListener("click", () => {
          //history.pushState({}, "", `./index.html?id=${e.id}`);
              idChange = e.id;
              fillForm(e);
          });
        ...
        row.querySelector(".btnDelete").addEventListener("click", async () => {
          ...
          const result = await deleteUser(e.id);
          alert(result.result);
        ...
        const mobileItem = `
            <div class="border p-4 rounded cursor-pointer bg-white"
                onclick='fillForm(${JSON.stringify(e)})'>
                <p><strong>Email:</strong> ${e.Email}</p>
                <p><strong>Tên đăng nhập:</strong> ${e.TenDangNhap }</p>
                <p><strong>Loại tài khoản:</strong> ${e.QuanLy ? "Quản lý" : "Khách hàng"}</p>
            </div>
        `;
...
const handleDele_Upload = async () => {
  ...
  if(btnSave){
    ...
    const body = {
      id: idChange,
      QuanLy: document.getElementById("role").value === "admin"
  };

    const password = document.getElementById("password").value;
    const TenDangNhap = document.getElementById("name").value;
    const Email = document.getElementById("email").value;


// Nếu có nhập mật khẩu thì mới gán vào body
  if (password.trim() !== "" ) {
    body.MatKhau = password;
  }
  if (TenDangNhap.trim() !== "" ) {
    body.TenDangNhap = TenDangNhap;
  }
  if (Email.trim() !== "" ) {
    body.Email = Email;
  }
